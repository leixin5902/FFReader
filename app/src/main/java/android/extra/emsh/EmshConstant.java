...
///////////////////////////////////////////////////////////////////////////////
	...
	// Send a request to power up UHF module
	/*
	{
	...
	*/
	...
	// Broadcast Emsh status
    /*
    {
        Intent intent = new Intent( EmshConstant.Action.INTENT_EMSH_BROADCAST );

        intent.addFlags( Intent.FLAG_RECEIVER_REGISTERED_ONLY | Intent.FLAG_EXCLUDE_STOPPED_PACKAGES );
        intent.putExtra( "SessionStatus", EmshBatteryStatus.SessionStatus ); 						// 和MCU通讯的session状态值
        intent.putExtra( "HardwareStatus", EmshBatteryStatus.HardwareStatus );						// 把枪硬件连接电平信号状态(0/1)
        intent.putExtra( "ProtocolVersion", EmshBatteryStatus.ProtocolVersion );					// 协议版本号
        intent.putExtra( "DeviceModelNumber", EmshBatteryStatus.DeviceModelNumber );				// 把枪设备型号
        intent.putExtra( "BatteryPowerMode", EmshBatteryStatus.BatteryPowerMode );					// 把枪电池当前状态
        intent.putExtra( "BatteryCapacityLevel", EmshBatteryStatus.BatteryCapacityLevel );			// 把枪电池剩余电量等级
        intent.putExtra( "BatteryCapacityPercent", EmshBatteryStatus.BatteryCapacityPercent );		// 把枪电池剩余电量百分比(0%)
        intent.putExtra( "BatteryTemperatureStatus", EmshBatteryStatus.BatteryTemperatureStatus );	// 把枪温度异常状态
        intent.putExtra( "BatteryTerminalVoltage", EmshBatteryStatus.BatteryTerminalVoltage );		// 把枪电池端电压(mV)
        intent.putExtra( "BatteryDischargeVoltage", EmshBatteryStatus.BatteryDischargeVoltage );	// 把枪放电输出电压(mV)
        intent.putExtra( "BatteryDischargeCurrent", EmshBatteryStatus.BatteryDischargeCurrent );	// 把枪放电输出电流(mA)
        intent.putExtra( "LatestUpdateUnixTime", EmshBatteryStatus.tmLatestUpdate );				// 数据更新时间戳
